...
# 把所有inputs从action.yaml里导出，用loop存到parser里
...
if __name__ == "__main__":
    arguments = vars(parser.parse_args()) # 可以在action.yaml里找到
    ...
    level = logging.DEBUG if debug else logging.WARNING # from import logging
    setup_logging(level, masked_values=[arguments['webhook_token']]) # from (log.py)
    ...
    workflow = Workflow.from_arguments(arguments) # from (types.py)
    webhook = Webhook.from_arguments(arguments) # from (types.py)
    ...
    if arguments.get("pull_request_payload", False): 
        ...
        pull_request = PullRequest.from_payload(arguments) # from (types.py)
    else:
        pull_request = PullRequest.from_arguments(arguments) # from (types.py)

    # Send webhook
    success = send_webhook(workflow, webhook, pull_request, dry_run) # from webhook 
